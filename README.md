...
`ipuOrders` はIPU申請フォーム用の購入候補。`manufacturer`, `itemName`, `specification`, `catalogNumber`, `quantity` を基本に、必要に応じて `unit`, `unitPriceYen`, `totalYen`, `primaryUser`, `sharedUsage`, `remarks`, `vendor`, `quoteNumber`, `quoteValidUntil` を保存する。単価・金額は申請フォームへそのまま貼れるカンマなしの半角数字表示にし、不明値は静的ファイルへ埋めず画面では「要確認」と表示する。
